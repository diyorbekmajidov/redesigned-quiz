...
class AuthCallbackView(View):
    ...
    def get(self, request):
        # Authorization code va state olish
        ...
        try:
            ...
            # TODO: Bu yerda Student modeliga saqlash va login qilish
            # from student.models import Student
            # student = self._get_or_create_student(user_details)
            # django.contrib.auth.login(request, student.user)
    ...
    def _get_or_create_student(self, user_details):
        ...
        hemis_id = user_details.get('id') or user_details.get('student_id')
        
        # Talabani topish yoki yaratish
        student, created = Student.objects.get_or_create(
            hemis_id=hemis_id,
            defaults={
                'student_name': user_details.get('full_name', ''),
                ...
                'faculty': user_details.get('faculty', ''),
                'level': user_details.get('level', '1'),
                'student_status': 'active',
                'payment_form': user_details.get('payment_form', 'contract'),
            }
        )
